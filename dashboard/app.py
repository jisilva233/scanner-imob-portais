...
from db.models import Base, PropertyListingDB, engine, make_fingerprint, PropertyListing, store_listings
...
def seed_demo_data():
    ...
    listings = []
    for title, price, bairro, area, quartos, fotos, portal, cidade, url in DEMO_DATA:
...
if df.empty:
    st.warning("Nenhum anúncio encontrado no banco de dados.")
    if st.button("📥 Carregar dados de demonstração (15 imóveis em São Paulo)"):
        seed_demo_data()
        st.cache_data.clear()
        st.rerun()
    st.stop()
