"""
scrapers/vivareal_scraper.py — Scraper do VivaReal usando Playwright.
"""
import asyncio
...
BASE_DOMAIN = "https://www.vivareal.com.br"
BASE_URL = f"{BASE_DOMAIN}/venda/{{city}}/"

SELECTORS = {
    "listing_cards": "[data-type='property']",
    "title": ".property-card__title",
    "price": ".property-card__price",
    "neighborhood": ".property-card__address",
    "area": "[data-testid='property-card/area']",
    "bedrooms": "[data-testid='property-card/bedrooms']",
    "photos_count": ".js-carousel-photos-count",
    "link": "a.property-card__content-link",
    "date": ".property-card__date",
...
class VivaRealScraper(BaseScraper):
    ...
    async def scrape(self, city: str, max_pages: int = 10) -> list[PropertyListing]:
        """Coleta anúncios do VivaReal para a cidade informada."""
            ...
            for page_num in range(1, max_pages + 1):
                ...
                for attempt in range(1, MAX_RETRIES + 1):
                    try:
                        ...
                        wait = 2 ** attempt
                        print(
                            f"[VIVAREAL] Tentativa {attempt}/{MAX_RETRIES} "
                ...
                if not loaded:
                    print(f"[VIVAREAL] Abandonando página {page_num} após {MAX_RETRIES} tentativas.")
                    break
        ...
        print(f"[VIVAREAL] Coletados {len(listings)} anúncios de {city}")
        return listings
    ...
    async def _extract_card(self, card, city: str) -> Optional[PropertyListing]:
        try:
            ...
            print(f"[VIVAREAL] Erro ao extrair card: {e}")
            return None
...
async def scrape_vivareal(city: str, max_pages: int = 10) -> list[PropertyListing]:
    return await VivaRealScraper().scrape(city, max_pages)
